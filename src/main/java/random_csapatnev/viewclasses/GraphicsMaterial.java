...
import random_csapatnev.modelclasses.MatEnum;
...
public class GraphicsMaterial extends JPanel {
	...
	public void paint(Graphics g) {
		super.paint(g);
		String amino = mat.getContainer().get(MatEnum.AMINOACID).toString();
		g.drawChars(amino.toCharArray(), 0, amino.length(), 0 + 2, graphicsParent.getHeight() - 2);
		String nucleo = mat.getContainer().get(MatEnum.NUCLEOTIDE).toString();
		g.drawChars(nucleo.toCharArray(), 0, nucleo.length(), graphicsParent.getWidth() / 2 + 2,
				graphicsParent.getHeight() - 2);
	}
}
